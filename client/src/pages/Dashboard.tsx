...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="min-h-screen py-6 px-4 sm:px-6 md:px-8 bg-gray-50">
      <div className="max-w-3xl lg:max-w-5xl mx-auto w-full">
        <h2 className="text-2xl sm:text-3xl font-bold text-indigo-800 mb-6 flex items-center gap-2">
          Dashboard
        ...
        {/* Calendar */}
        <section className="bg-white p-3 sm:p-6 rounded-2xl shadow-md mb-8 w-full overflow-x-auto">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-4">
            <h3 className="text-xl sm:text-2xl font-semibold text-indigo-700">📅 Calendar Overview</h3>
          ...
          <div className="min-w-[320px] w-full">
            <FullCalendar
              ...
              height="auto"
              aspectRatio={window.innerWidth < 640 ? 1 : 1.8} // tighter layout on small screens
              contentHeight={window.innerWidth < 640 ? 300 : 400}
              dayMaxEventRows={2}
              eventClick={(info) => {
        ...
        {/* To-Do List */}
        <section className="bg-white p-4 sm:p-6 rounded-2xl shadow mb-8 w-full">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-4">
            <h3 className="text-xl sm:text-2xl font-semibold text-indigo-700">📝 To-Do List</h3>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-4 w-full">
            <input
              ...
              className="w-full sm:flex-grow border border-indigo-300 px-3 py-2 rounded-lg focus:ring-2 ring-indigo-400 transition"
            />
            <Tooltip content="Add this task">
              <button
                onClick={handleAddTask}
                className="w-full sm:w-auto px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-semibold transition"
              >
                Add
              </button>
            </Tooltip>
          ...
          <ul className="space-y-2 max-h-48 overflow-auto">
            {tasks.length === 0 ? (
              ...
              tasks.map((t: Task) => (
                <li
                  key={t._id}
                  className="flex items-center gap-2 cursor-pointer break-words"
                  onClick={() => {
                    toggleTask(t._id);
                ...
                >
                  ...
                  <span className={`${t.done ? "line-through text-gray-400" : "text-indigo-800"}`}>{t.text}</span>
                </li>
              ))
            )}
          </ul>
          <div className="mt-4 text-right">
            <Tooltip content="Go to full To‑Do page">
              <Link to="/todo" className="text-indigo-600 hover:underline font-medium">Manage All Tasks →</Link>
            </Tooltip>
          </div>
        </section>
        ...
        <section className="bg-white p-4 sm:p-6 rounded-2xl shadow mb-8 w-full">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-4">
            <h3 className="text-xl sm:text-2xl font-semibold text-indigo-700">📂 Resumes</h3>
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      ...
                      <Tooltip content="Analyze resume for skills">
                        <button
                          ...
                          className="px-4 py-2 bg-purple-100 text-purple-800 rounded-lg font-medium hover:bg-purple-200 transition">Analyze</button>
                      </Tooltip>
                    </div>
                  </div>
                  {analysisResults[_id] && (
                    <div className="mt-3 p-4 bg-white rounded-lg border text-sm overflow-x-auto">
                      <p className="font-semibold text-indigo-800">Skills Found:</p>
                      <p className="text-gray-700 mb-2 whitespace-normal">{analysisResults[_id].existing_skills.join(", ")}</p>
                      <p className="font-semibold text-indigo-800">🔮 Suggested Skills:</p>
                      <p className="text-gray-700 whitespace-normal">{analysisResults[_id].suggested_skills.join(", ")}</p>
                    </div>
                  )}
                </li>
              ))}
          ...
          <div className="mt-4 text-right">
            <Tooltip content="Go to full Resume Manager">
              <Link to="/resume" className="text-indigo-600 hover:underline font-medium">Manage All Resumes →</Link>
            </Tooltip>
          </div>
        </section>
        ...
        <section className="bg-white p-4 sm:p-6 rounded-2xl shadow mb-8 w-full">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-4">
            <h3 className="text-xl sm:text-2xl font-semibold text-indigo-700">💻 Skill Suggestions</h3>
            <Tooltip content="Get personalized recommendations">
              <span className="w-6 h-6 text-xs bg-indigo-800 text-white rounded-full inline-flex justify-center items-center cursor-default">?</span>
            </Tooltip>
          </div>
          <Tooltip content="Visit Skill Suggestions page">
            <Link to="/skills" className="inline-block px-6 py-2 bg-purple-600 text-white rounded-lg font-semibold hover:bg-purple-700 transition">View Suggestions</Link>
          </Tooltip>
        </section>
        ...
        <section className="bg-white p-4 sm:p-6 rounded-2xl shadow mb-8 w-full">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-4">
            <h3 className="text-xl sm:text-2xl font-semibold text-indigo-700">🔗 Quick Links</h3>
