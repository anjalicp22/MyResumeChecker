...
import SkillAnalysisBox from "../components/SkillAnalysisBox";
import RecommendResumeBox from "../components/RecommendResumeBox";
...
const ApplicationForm: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [resApp, resResume] = await Promise.all([
          api.get("/api/applications", { headers: { Authorization: `Bearer ${token}` } }),
          api.get("/api/resumes", { headers: { Authorization: `Bearer ${token}` } }),
        ]);
        setApplications(resApp.data);
        setResumes(resResume.data);
        if (resResume.data.length > 0) {
          setSelectedResumeId(resResume.data[resResume.data.length - 1]._id);
        }
      } catch (err) {
        toast.error("Failed to load initial data.");
        logError("initialLoad", err);
      } finally {
        setLoadingInitial(false);
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...formData,
        analysisResult: analysis,
        resumeUsed: selectedResumeId || undefined,
      };
      await api.post("/api/applications", payload, { headers: { Authorization: `Bearer ${token}` } });
      toast.success("Application saved.");
      setFormData(defaultForm);
  ...
  const handleRecommend = async () => {
    ...
    setRecommending(true);
    setRecommendResult(null);
    const resumeScores: Record<string, number> = {};
    ...
    for (const r of resumes) {
      try {
        const resp = await getSkillsForResume(r._id, token!);
        const skills = resp.data?.[0]?.skills || [];
        localSkillsMap[r._id] = skills;
        const score = scoreResumeSkills(analysis.required_skills, skills);
        resumeScores[r._id] = score;
      } catch (err) {
        logError("handleRecommend > resume loop", err);
      }
    }

    setResumeSkillsMap(localSkillsMap);

    const bestEntry = Object.entries(resumeScores).sort((a, b) => b[1] - a[1])[0];
    if (bestEntry) {
      const [rid, score] = bestEntry;
      const bestResume = resumes.find(r => r._id === rid);
      if (bestResume) {
        setSelectedResumeId(rid);
        setResumeSkills(localSkillsMap[rid]);
        setRecommendResult({ rid, filename: bestResume.filename, score });
        toast.success(`Best resume: ${bestResume.filename} (score ${score})`);
      }
    } else {
      toast.error("No suitable resume found.");
    }
  ...
  return (
    <div className="min-h-screen py-10 px-4">
      <section className="bg-white p-8 rounded-xl shadow-lg space-y-6">
        <ActionButtons
          ...
          handleReset={() => {
            setFormData(defaultForm);
            setAnalysis(null);
            setSelectedResumeId(resumes.length ? resumes[resumes.length - 1]._id : null);
            setResumeSkills([]);
            setErrors({});
          }}
        />
        ...
        {analysis && (
          <SkillAnalysisBox analysis={analysis} resumeSkills={resumeSkills} />
        )}
      ...
      <SavedApplications
        ...
        showAllDetailsMap={showAllDetailsMap}
        resumes={resumes}
        resumeSkillsMap={resumeSkillsMap}
        submitting={submitting}
